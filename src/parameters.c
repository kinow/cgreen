...
CgreenVector *create_vector_of_names(const char *parameters) {
    ...
    tokens = tokenise_by_commas_and_whitespace(_strdup(parameters));
#else
    tokens = tokenise_by_commas_and_whitespace(strdup(parameters));
#endif
    token = tokens;
    while (token < tokens + strlen(parameters)) {
        token = strip_d_macro(strip_box_double(skip_nulls(token)));
#if defined WINCE || defined WIN32
        cgreen_vector_add(names, _strdup(token));
#else
